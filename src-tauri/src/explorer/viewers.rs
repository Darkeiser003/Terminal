...
pub fn default_linux_file_manager() -> Option<&'static FileManager> {
    ...
    file_manager_for_desktop(&desktop, &installed).or_else(|| {
        // Algunos compositoras (Hyprland, sway...) registran un lanzador de
        // terminal como predeterminado para `inode/directory`. No es un gestor
        // de archivos y `gio open` termina sin mostrar nada. Si la asociación
        // no identifica un gestor conocido, usar uno instalado directamente es
        // más fiable que delegar de nuevo en la misma asociación rota.
        LINUX_MANAGERS.iter().find(|manager| installed(manager.cmd))
    })
}
