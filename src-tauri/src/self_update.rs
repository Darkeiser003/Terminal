...
/// `v1.4.2`, `1.4.2` y `1.4.2-beta` dan los mismos tres números.
///
...
/// por orden basta. Tratar `1.4.2-beta` como igual a `1.4.2` es lo prudente:
...
pub fn asset_for_platform<'a>(names: &[&'a str]) -> Option<&'a str> {
    let (extension, excluir): (&str, &[&str]) = if cfg!(windows) {
        (".zip", &["source", "linux", "macos", "darwin"])
    } else if cfg!(target_os = "macos") {
        (".dmg", &["source", "windows", "linux"])
    } else {
...
mod tests {
    ...
    fn una_version_se_lee_con_o_sin_la_uve_de_delante() {
        assert_eq!(Version::parse("v1.4.2"), Version::parse("1.4.2"));
    ...
    fn la_misma_version_no_se_ofrece_como_actualizacion() {
        let actual = Version::parse("1.4.2").unwrap();
        ...
        assert!(!Version::parse("1.4.2-beta").unwrap().is_newer_than(&actual));
    }
