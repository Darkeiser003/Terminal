...
fn projects_state(state: &AppState) -> ProjectsState {
    ...
    ProjectsState {
        ...
        brand: if crate::platform::host().is_windows() {
            pins.brand.clone()
        } else {
...
fn github_client() -> &'static github::GithubClient {
    github::shared_client()
}
    ...
    if env.transport != Transport::Wsl && !crate::path_env::is_tool_installed("git") {
        return GitRunResult {
            ...
            suggestion: Some(crate::command_not_found::ToolSuggestion {
                tool: "git".to_string(),
                label: "Git".to_string(),
                action_id: crate::platform::host()
                    .is_windows()
                    .then(|| "winget-git".to_string()),
            }),
            ..Default::default()
        };
    }
...
mod tests {
    ...
    #[test]
    fn el_catalogo_de_fabrica_ancla_el_perfil_del_proyecto_y_conserva_los_creditos() {
        let catalog = github::default_catalog();
    ...
    #[test]
    fn el_catalogo_separa_el_credito_de_los_proyectos_anclados() {
        let catalog = github::default_catalog();
        assert!(catalog.owners.is_empty());
        assert_eq!(catalog.fixed_profiles, vec!["Darkeiser003"]);
        assert!(catalog.repositories.is_empty());
        ...
        assert!(catalog.project_leads.is_empty());
        assert_eq!(
            catalog.self_repository.as_deref(),
