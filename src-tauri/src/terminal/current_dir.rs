...
// En Windows Starship conserva la ruta nativa en la línea de contexto:
// `C:\\proyecto con espacios main` y deja el prompt editable en la siguiente.
// La rama puede ir detrás de una ruta con espacios, así que no se puede cortar
// simplemente en el primer espacio.
static STARSHIP_WINDOWS_PROMPT: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?m)(?:^|\n)((?:[A-Za-z]:\\|\\\\)[^\n]+)\n[ \t]*(?:❯|➜|>)\s*$").unwrap()
});
...
fn resolve_starship_path(raw: &str, env: &Environment) -> Option<String> {
    ...
    loop {
        ...
        if let Some(mapped) = map_remote_path(candidate, env) {
            if starship_path_is_dir(&mapped, env) {
                return Some(mapped);
            }
        }
...
fn starship_path_is_dir(mapped: &str, env: &Environment) -> bool {
    // Un cmd ejecutado con Wine imprime rutas de Windows, pero Rust corre en
    // el host Unix. `Path::new("Z:\\tmp\\x")` no puede comprobarlas; Z: es
    // precisamente el puente de Wine hacia la raíz del host.
    if env.transport == Transport::Wine {
        return wine_path_to_posix(mapped).is_some_and(|path| Path::new(&path).is_dir());
    }
    Path::new(mapped).is_dir()
}
...
pub fn detect_current_directory(
    ...
    if let Some(found) = last_capture(text, &STARSHIP_WINDOWS_PROMPT) {
        return resolve_starship_path(&found, env).and_then(|path| {
            if env.transport == Transport::Wine {
                wine_path_to_posix(&path)
            } else {
                Some(path)
            }
        });
...
mod tests {
    ...
    #[test]
    fn starship_windows_conserva_los_espacios_bajo_wine() {
        // La prueba valida la unidad Z: que Wine expone al host. Un test
        // Windows nativo no tiene por qué disponer de esa unidad, y sus
        // temporales C:\\ no representan una ruta visible para este
        // transporte.
        if cfg!(windows) && std::env::var("LTERMINAL_TEST_UNDER_WINE").as_deref() != Ok("1") {
            return;
        }
        let root = if cfg!(windows) {
            tempfile::tempdir_in(r"Z:\tmp").unwrap()
        } else {
        ...
        let windows_path = if cfg!(windows) {
            ...
            format!("Z:{}", directory.to_string_lossy().replace('/', "\\"))
        };
        let out = format!("{} main\n❯ ", windows_path);
        let expected = if cfg!(windows) {
            wine_path_to_posix(&windows_path).unwrap()
        } else {
            directory.to_string_lossy().to_string()
        };

        assert_eq!(
            detect_current_directory(&out, &env(Transport::Wine), None),
            Some(expected)
        );
    }
    ...
    #[cfg(windows)]
    #[test]
    fn starship_windows_conserva_los_espacios_en_windows_nativo() {
        let root = tempfile::tempdir().unwrap();
        let directory = root.path().join("proyecto con espacios");
        std::fs::create_dir(&directory).unwrap();
        let out = format!("{} main\n❯ ", directory.display());
