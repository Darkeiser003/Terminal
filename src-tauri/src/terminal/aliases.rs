...
fn sysinfo_alias_line(
    ...
    // `bannerclear` comienza en HOME y borra pantalla e historial antes de
    // dibujar el bloque; así `sysinfo` no deja restos del fastfetch anterior.
    let display_path = banner_clear_path.or(banner_path);
    if kind == ShellKind::Cmd {
        return match display_path {
    ...
    let fallback = match display_path {
...
// Las descripciones empiezan en el siguiente tabulador común después del
// comando más largo. Cada fila usa solo los tabuladores que necesita para
// llegar a esa columna; así no se mezclan espacios de relleno con el texto.
const HELP_INDENT_WIDTH: usize = 4;
const HELP_TAB_WIDTH: usize = 8;
const HELP_DESCRIPTION_COLUMN: usize = 64;

fn help_row(command: &str, description: &str) -> String {
    let width = command.chars().count();
    let command_end = HELP_INDENT_WIDTH + width;
    let tab_count = if command_end < HELP_DESCRIPTION_COLUMN {
        (HELP_DESCRIPTION_COLUMN - command_end).div_ceil(HELP_TAB_WIDTH)
    } else {
        1
    };
    let tabs = "\t".repeat(tab_count.max(1));
    format!(
        "    {HELP_CMD_COLOR}{command}{HELP_RESET}{tabs}{}",
        description.trim()
    )
}
...
pub fn build_help_topic_text(
    ...
    lines.push(help_row("ayuda <seccion>", "solo esa sección"));
    lines.push("    secciones: paquetes, sesion, internos, alias, biblioteca, menus, plugins, soporte, creditos".to_string());

    // Ninguna fila se envuelve artificialmente: las descripciones conservan
    // su línea lógica completa y el terminal decide su desplazamiento
    // horizontal. Los saltos que quedan son solo separadores semánticos.
...
pub fn build_init_script(
    ...
    // Zsh expande alias al leer el nombre de una función. Si el perfil del
    // usuario, por ejemplo, define `update='sudo pacman -Syu'`, la declaración
    // siguiente `update() { ... }` se convierte en sintaxis inválida y deja un
    // error visible al abrir la shell. Estos nombres los administra LTerminal
    // a continuación, así que solo en Zsh quitamos sus alias previos antes de
    // declarar las funciones propias.
    if kind == ShellKind::Zsh {
        lines.push(
            "unalias clear cls sysinfo ayuda help install update upgrade uninstall remove search 2>/dev/null || true"
                .to_string(),
        );
    }
...
mod tests {
    ...
    #[test]
    fn zsh_limpia_aliases_de_usuario_antes_de_declarar_las_funciones_de_la_app() {
        let zsh = build(ShellKind::Zsh, &options(Transport::Native));
        let unalias = zsh
            .content
            .find("unalias clear cls sysinfo ayuda help install update upgrade uninstall remove search")
            .expect("Zsh debe desactivar los alias que colisionan con funciones de la app");
        for function in [
            "clear() {",
            "cls() {",
            "sysinfo() {",
            ...
            let declaration = zsh
                .content
                .find(function)
                .unwrap_or_else(|| panic!("Zsh no declara la función {function}"));
            assert!(
                unalias < declaration,
                "Zsh debe eliminar posibles alias antes de {function}"
            );
        }

        let bash = build(ShellKind::Bash, &options(Transport::Native));
        assert!(!bash.content.contains("unalias clear cls sysinfo"));
    }
    ...
    fn clear_reimprime_el_banner_a_traves_de_un_indicador() {
        ...
        assert!(script
            .content
            .contains("doskey sysinfo=type \"C:\\temp\\bannerclear.txt\""));
    }
    ...
    fn sin_banner_sysinfo_cae_al_comando_del_sistema() {
        assert!(
            sysinfo_alias_line(ShellKind::Cmd, None, None, Transport::Native)
                .contains("systeminfo")
        );
        assert!(
            sysinfo_alias_line(ShellKind::Bash, None, None, Transport::Native).contains("uname -a")
        );
        assert!(
            sysinfo_alias_line(ShellKind::Powershell, None, None, Transport::Native)
    ...
    fn sysinfo_no_usa_fastfetch_ni_neofetch() {
        let line = sysinfo_alias_line(ShellKind::Bash, None, None, Transport::Native);
    ...
    #[test]
    fn sysinfo_prefiere_el_banner_que_limpia_toda_la_vista() {
        let line = sysinfo_alias_line(
            ShellKind::Bash,
            Some("C:\\temp\\banner.txt"),
            Some("C:\\temp\\bannerclear.txt"),
            Transport::Wsl,
        );
        assert!(line.contains("/mnt/c/temp/bannerclear.txt"), "{line}");
        assert!(!line.contains("/mnt/c/temp/banner.txt"), "{line}");
    }
    ...
    #[test]
    fn la_ayuda_mantiene_comando_y_descripcion_en_una_fila_tabular() {
        let row = help_row(
            ":panel <settings|deps|projects|scripts|explorer|close>",
            "Abre, cierra o enumera los paneles principales.",
        );
        assert!(!row.contains('\n'));
        assert!(row.contains('\t'));
        assert!(row.contains(":panel <settings|deps|projects|scripts|explorer|close>"));
        assert!(row.ends_with("Abre, cierra o enumera los paneles principales."));
    }
    ...
    fn las_columnas_de_la_ayuda_quedan_alineadas() {
        ...
        let visible_column = |text: &str, marker: &str| {
            let without_color = text.replace(HELP_CMD_COLOR, "").replace(HELP_RESET, "");
            without_color[..without_color.find(marker).unwrap()]
                .chars()
                .fold(0, |column, character| {
                    if character == '\t' {
                        ((column / HELP_TAB_WIDTH) + 1) * HELP_TAB_WIDTH
                    } else {
                        column + 1
                    }
                })
        };
        // Las dos descripciones empiezan en la misma columna y el separador
        // ya no depende de espacios de relleno.
        assert_eq!(
            visible_column(&corta, "Esta"),
            visible_column(&larga, "Desinstala.")
        );
        assert!(corta.contains('\t'));
        assert!(larga.contains('\t'));
    }

    #[test]
    fn la_ayuda_interna_conserva_filas_largas_para_el_scroll_horizontal() {
        ...
        let sin_ansi = texto
            ...
            .replace("\x1b[1;92m", "")
            .replace(HELP_RESET, "");
        let filas_tabulares = sin_ansi
            .lines()
            .filter(|fila| fila.contains('\t'))
            .collect::<Vec<_>>();
        assert!(!filas_tabulares.is_empty());
        assert!(filas_tabulares
            .iter()
            .any(|fila| fila.contains(":panel <settings|deps|projects|scripts|explorer|close>")));
        assert!(filas_tabulares.iter().any(|fila| fila.chars().count() > 72));
    }
