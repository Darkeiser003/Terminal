...
/// La creación del PTY puede seguir en segundo plano en el uso normal; los
/// smokes de release reintentan mientras llega la sesión y solo aceptan true
/// cuando hay PTY y la ventana principal se pudo mostrar.
#[tauri::command]
pub fn frontend_ready(
    ...
    if !state.tabs.exists(&tab_id) {
        // La creación del PTY es asíncrona: que todavía no haya una sesión no
        // es un error, pero una pestaña inexistente sí indica una carrera o un
        // frontend desincronizado.
        log_error!(
            "Frontend preparado para una pestaña inexistente",
            serde_json::json!({ "tabId": tab_id, "smokeToken": smoke_token })
        );
        return Err("La pestaña ya no existe".into());
    }
    let session_ready = state.tabs.has_session(&tab_id);
    if !session_ready {
        // La creación del PTY es asíncrona y puede terminar después de que el
        // xterm ya esté montado. El frontend muestra la ventana aparte y
        // reintenta aquí; no libera entrada ni registra un éxito prematuro.
        return Ok(false);
    }
    ...
    if std::env::var("LTERMINAL_WINE_SMOKE").as_deref() != Ok("1") {
        let window = app
            .get_webview_window("main")
            .ok_or_else(|| "No se encontró la ventana principal inicial".to_string())?;
        window
            .show()
            .map_err(|error| format!("No se pudo mostrar la ventana inicial: {error}"))?;
    } else {
        // Wine puede devolver BadWindow al hacer show/unmap de una ventana Tao
        // creada inicialmente oculta. El WebView ya se ha inicializado; la
        // visibilidad nativa se valida en Windows real.
        log_info!("Ventana Wine omitida durante el smoke gráfico");
    }
    // Los scripts de release consumen este marcador; escribirlo tras `show`
    // evita dar por bueno un smoke si falló la presentación de la ventana.
    log_info!(
...
/// Muestra la ventana cuando el xterm ya está montado, antes de que la PTY
/// termine de arrancar. También sirve para revelar un error de carga inicial.
#[tauri::command]
