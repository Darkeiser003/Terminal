...
mod tests {
    ...
    #[test]
    fn el_catalogo_declara_cual_es_el_repositorio_de_esta_aplicacion() {
        // Sin esto la app no puede distinguir su propia release de la de
        // cualquier otro proyecto anclado en el panel.
        assert!(crate::github::default_catalog().self_repository.is_some());
    }

    #[test]
    fn el_repositorio_propio_se_reconoce_sin_distinguir_mayusculas() {
        let propio = crate::github::default_catalog().self_repository.unwrap();
        assert!(is_self_repository(&propio));
        assert!(is_self_repository(&propio.to_uppercase()));
