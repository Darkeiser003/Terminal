...
/// LTerminal pinta su propio banner de bienvenida. Fish, sin embargo, puede
/// ejecutar `fish_greeting` desde la configuración del usuario antes de leer
/// el inicializador de la pestaña; en CachyOS ese saludo lanza `fastfetch` y
/// bajo un PTY puede tardar varios segundos. `-C` se ejecuta después de leer la
/// configuración, por lo que solo desactiva el saludo de esta sesión y no
/// modifica ningún archivo ni preferencia del usuario.
const FISH_ARGS: &[&str] = &["-i", "-C", "functions -e fish_greeting"];
...
mod tests {
    ...
    #[test]
    fn fish_carga_la_configuracion_pero_no_ejecuta_el_saludo_lento() {
        assert_eq!(FISH_ARGS, &["-i", "-C", "functions -e fish_greeting"]);
    }
