...
# Git Bash/MSYS/Cygwin también proporcionan bash en Windows, pero no son un
# entorno Linux: aquí no existen WebKitGTK, AppImage ni el toolchain Linux que
# este script necesita. Detectarlo al principio evita un fallo confuso de
# paquetes y dirige al builder nativo de Windows.
case "$(uname -s 2>/dev/null || printf '')" in
    MINGW*|MSYS*|CYGWIN*)
        echo "ERROR: linux/build.sh solo compila la release Linux y no se puede usar como builder nativo de Windows." >&2
        echo "       Ejecuta desde PowerShell: windows\\build.ps1 -NonInteractive -NoRun" >&2
        echo "       O desde CMD: windows\\build.bat -NonInteractive -NoRun" >&2
        echo "       Si estás usando WSL, ejecuta este script dentro de WSL, no desde Git Bash." >&2
        exit 2
        ;;
esac
...
backup_version_manifests() {
    ...
    for path in "${version_manifest_paths[@]}"; do
        [ -f "$path" ] || {
            ...
            VERSION_BACKUP_DIR=""
            fail "No existe el manifiesto de versión $path."
        }
...
smoke_log_ready() {
    ...
    # Exigir los hitos reales evita que el build pase solo porque WebView llegó
    # a pintar una ventana o porque quedó un log antiguo.
    grep -Fq "Ventana inicial preparada" "$path" || return 1
    grep -Fq "pty spawneado" "$path" || return 1
    grep -Fq "Frontend y terminal preparados" "$path"
}
...
on_exit() {
    ...
    if ! restore_version_manifests; then
        ...
        # terminó con código cero. `exit` propaga el fallo de restauración.
        exit 1
    fi
    return "$exit_code"
}
...
if [ "${LTERMINAL_TEST_FAIL_AFTER_VERSION:-0}" = "1" ]; then
    err "Fallo de prueba solicitado después de aplicar la versión."
    exit 1
fi
...
if [ "${APPIMAGE_ARCH:-x86_64}" = "x86_64" ] && [ -x "$LINUXDEPLOY_CACHE" ]; then
    # linuxdeploy escribe su versión en stderr; capturar ambas salidas evita
    # tratar una build compatible en caché como obsoleta e intentar descargarla.
    LINUXDEPLOY_VERSION="$(APPIMAGE_EXTRACT_AND_RUN=1 "$LINUXDEPLOY_CACHE" --version 2>&1 || true)"
        ...
        if ! command -v tauri-driver >/dev/null 2>&1; then
            ...
            if [ -z "$E2E_DRIVER_PATH" ]; then
                ...
                if ! APPIMAGE_EXTRACT_AND_RUN="${APPIMAGE_EXTRACT_AND_RUN:-1}" \
                    TAURI_NATIVE_DRIVER="$E2E_DRIVER_PATH" \
