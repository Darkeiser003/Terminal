<script lang="ts">
    ...
    async function save(event: SubmitEvent): Promise<void> {
        ...
        try {
            ...
            statusError = true;
            status =
                app.t('settings.saveFailed', 'No se pudieron guardar las preferencias.') +
                ' ' +
                String(cause);
        } finally {
            saving = false;
    ...
    async function reset(): Promise<void> {
        ...
        try {
            ...
            statusError = true;
            status =
                app.t('settings.resetFailed', 'No se pudieron restablecer las preferencias.') +
                ' ' +
                String(cause);
        } finally {
            saving = false;
    ...
    async function installUpdate(): Promise<void> {
        ...
        try {
            ...
            statusError = true;
            status = result.error ?? app.t('update.failed', 'No se pudo actualizar.');
        } catch (cause) {
            statusError = true;
            status = String(cause);
        } finally {
    ...
    const version = $derived(app.appInfo?.version ? `v${app.appInfo.version}` : '');
    const platform = $derived(
        app.appInfo ? (PLATFORM_NAMES[app.appInfo.platform] ?? app.appInfo.platform) : ''
    );
    ...
    function roleOf(developer: string): string {
        const owners = (app.appInfo?.owners ?? []).map((login) => login.toLowerCase());
...
<Panel
    ...
    subtitle={status || app.t('settings.subtitle', 'Personaliza la interfaz sin cambiar el sistema.')}
    error={statusError}
    width={610}
>
    {#snippet header()}
        <div class="tabs" role="tablist" aria-label={app.t('settings.sections', 'Secciones de preferencias')}>
    ...
    {#if !draft}
        <div class="loading">{app.t('settings.loading', 'Cargando preferencias…')}</div>
    {:else}
        <form onsubmit={save}>
            {#if section === 'appearance'}
                <section>
                    ...
                    <div class="themes">
                        {#each app.themes as theme (theme.id)}
                            ...
                            >
                                ...
                                <span
                                    class="swatch"
                                    style="background: linear-gradient(135deg, {theme.palette
                                        .background} 0 64%, {theme.palette.accent} 64%)"
                                ></span>
                    ...
                    >
                        <span style="color: {draft.accentColor}">usuario@equipo</span
                        ><strong style="color: {draft.accentColor}">:~/proyecto</strong
                    ...
                    <div class="grid">
                        <label class="field">
                            ...
                            <input type="color" bind:value={draft.accentColor} />
                        </label>
                        <label class="field">
                            <span>{app.t('settings.terminalBg', 'Fondo terminal')}</span>
                            <input type="color" bind:value={draft.terminalBackground} />
                        </label>
                        <label class="field">
                            <span>{app.t('settings.terminalFg', 'Texto terminal')}</span>
                            <input type="color" bind:value={draft.terminalForeground} />
                        </label>
                        <label class="field">
                            <span>{app.t('settings.density', 'Densidad de interfaz')}</span>
                            <select bind:value={draft.uiDensity}>
                                <option value="comfortable">
                                    {app.t('settings.densityComfortable', 'Cómoda')}
                                </option>
                                <option value="compact">
                                    {app.t('settings.densityCompact', 'Compacta')}
                                </option>
                            </select>
                        </label>
                    </div>
            ...
            {#if section === 'terminal'}
                <section>
                    <div class="heading">
                        <strong>{app.t('settings.textCursor', 'Texto y cursor')}</strong>
                        <span>
                            {app.t('settings.textCursorHint', 'Ajustes aplicados a todas las pestañas abiertas.')}
                        </span>
                    </div>
                    ...
                    <div class="grid">
                        ...
                        <label class="field">
                            ...
                            <input type="number" min="10" max="24" step="1" bind:value={draft.terminalFontSize} />
                        </label>
                        <label class="field">
                            <span>{app.t('settings.lineHeight', 'Altura de línea')}</span>
                            <input type="number" min="0.9" max="1.8" step="0.05" bind:value={draft.terminalLineHeight} />
                        </label>
                        <label class="field">
                            <span>{app.t('settings.letterSpacing', 'Espaciado')}</span>
                            <input type="number" min="-1" max="3" step="0.1" bind:value={draft.terminalLetterSpacing} />
                        </label>
                        <label class="field">
                            <span>{app.t('settings.cursor', 'Cursor')}</span>
                            <select bind:value={draft.terminalCursorStyle}>
                                <option value="block">{app.t('settings.cursorBlock', 'Bloque')}</option>
                                <option value="bar">{app.t('settings.cursorBar', 'Barra')}</option>
                                <option value="underline">{app.t('settings.cursorUnderline', 'Subrayado')}</option>
                                <option value="beam">{app.t('settings.cursorBeam', 'Barra gruesa')}</option>
                                <option value="underline-thick">
                                    {app.t('settings.cursorUnderlineThick', 'Subrayado grueso')}
                                </option>
                            </select>
                        </label>
                        <label class="field">
                            ...
                            <select bind:value={draft.terminalFontWeight}>
                                <option value="normal">{app.t('settings.fontWeightNormal', 'Normal')}</option>
                                <option value="bold">{app.t('settings.fontWeightBold', 'Negrita')}</option>
                            </select>
                        </label>
                        <label class="field">
                            <span>{app.t('settings.padding', 'Margen interior')}</span>
                            <input type="number" min="4" max="24" step="1" bind:value={draft.terminalPadding} />
                        </label>
                        <label class="field">
                            <span>{app.t('settings.scrollSensitivity', 'Velocidad de rueda')}</span>
                            <input
                                type="number"
                                min="1"
                                max="10"
                        ...
                        <label class="field span-2">
                            <span>{app.t('settings.scrollback', 'Líneas de historial')}</span>
                            <input
                                type="number"
                                min="1000"
                                max="100000"
                    ...
                    <label class="check">
                        <input type="checkbox" bind:checked={draft.terminalCursorBlink} />
                        <span>
                            <strong>{app.t('settings.cursorBlink', 'Cursor parpadeante')}</strong>
                            <small>{app.t('settings.cursorBlinkHint', 'Facilita localizar el punto de escritura.')}</small>
                        </span>
                    </label>
                    <label class="check">
                        <input type="checkbox" bind:checked={draft.copyOnSelect} />
                        <span>
                            <strong>{app.t('settings.copyOnSelect', 'Copiar al seleccionar')}</strong>
                            <small>
                                {app.t('settings.copyOnSelectHint', 'Al soltar el ratón, lo seleccionado va al portapapeles.')}
                            </small>
                        </span>
                    </label>
                    <label class="check">
                        <input type="checkbox" bind:checked={draft.showSystemBanner} />
                        <span>
                            <strong>{app.t('settings.showBanner', 'Mostrar información del sistema')}</strong>
                            <small>{app.t('settings.showBannerHint', 'Banner al crear una sesión nueva.')}</small>
                        </span>
                    </label>
                </section>
            {/if}
            ...
            {#if section === 'behavior'}
                <section>
                    <div class="heading">
                        <strong>{app.t('settings.startupPanels', 'Inicio y paneles')}</strong>
                        <span>{app.t('settings.startupPanelsHint', 'Solo cambia el comportamiento de la aplicación.')}</span>
                    </div>
                        ...
                        <select bind:value={draft.language}>
                            {#each app.languages as language (language.id)}
                                ...
                                <option value={language.id}>
                                    {language.id === 'auto'
                                        ? app.t('settings.languageAuto', language.label)
                                        : language.label}
                                </option>
                            {/each}
                        </select>
                    ...
                    <label class="field wide">
                        <span>{app.t('settings.startupEnv', 'Entorno al iniciar')}</span>
                        <select bind:value={draft.defaultEnvironmentId}>
                            <option value="">{app.t('settings.envAuto', 'Automático según el sistema')}</option>
                    ...
                    <label class="check">
                        <input type="checkbox" bind:checked={draft.autoStartDocker} />
                        <span>
                            <strong>{app.t('settings.autoDocker', 'Iniciar Docker automáticamente')}</strong>
                            <small>{app.t('settings.autoDockerHint', 'Solo cuando está instalado y el daemon no responde.')}</small>
                        </span>
                    </label>
                    <label class="check">
                        <input type="checkbox" bind:checked={draft.exclusiveAccordionGroups} />
                        <span>
                            <strong>{app.t('settings.exclusiveGroups', 'Una lista abierta por panel')}</strong>
                            <small>{app.t('settings.exclusiveGroupsHint', 'Al abrir una, cierra la anterior.')}</small>
                        </span>
                    </label>
                    <label class="check">
                        <input type="checkbox" bind:checked={draft.autoOpenFirstGroup} />
                        <span>
                            <strong>{app.t('settings.autoOpenFirst', 'Abrir la primera lista')}</strong>
                            <small>
                                {app.t('settings.autoOpenFirstHint', 'Desactivado mantiene Docker y los demás grupos cerrados.')}
                            </small>
                        </span>
                    </label>
                </section>
                ...
                <section>
                    <div class="about-card">
                        ...
                        <div>
                            ...
                            <span>{[version, platform].filter(Boolean).join(' · ')}</span>
                        </div>
                    </div>

                    <div class="heading">
                        <strong>{app.t('settings.update', 'Actualización')}</strong>
                        <span>
                            {#if updating}
                                {app.t('update.checking', 'Consultando la última versión…')}
                            ...
                            {:else if update}
                                {app.t('update.upToDate', 'Estás en la versión más reciente.')}
                            {/if}
                        </span>
                    </div>
                    <div class="update-row">
                        <button type="button" class="secondary" disabled={updating} onclick={checkUpdate}>
                        ...
                        {#if update?.available && update.canSelfUpdate}
                            <button type="button" class="primary" disabled={updating} onclick={installUpdate}>
                    ...
                    <div class="heading">
                        <strong>{app.t('settings.developers', 'Desarrolladores')}</strong>
                        <span>{app.t('settings.developersHint', 'Créditos definidos en el catálogo de distribución.')}</span>
                    </div>
                    <div class="developers">
                        {#each app.appInfo?.developers ?? [] as developer (developer)}
                            <button
                                ...
                                title={app
                                    ...
                                    .replace('{url}', `https://github.com/${developer}`)}
                        ...
                        {:else}
                            <span class="field-hint">
                                {app.t('settings.developersPending', 'Pendiente de completar.')}
                            </span>
                        {/each}
                    </div>

                    <div class="heading">
                        <strong>{app.t('settings.localConfig', 'Configuración local')}</strong>
                    </div>
                    <div class="path" title={app.appInfo?.settingsPath ?? ''}>
                        {app.t('settings.file', 'Archivo')}: {app.appInfo?.settingsPath ?? ''}
                    </div>
                </section>
            {/if}

            <div class="footer">
                <button type="button" class="secondary" disabled={saving} onclick={reset}>
