...
/** Los cuatro atajos de navegación de panel reservan el Ctrl físico izquierdo
 *  cuando incluyen Ctrl. El estado se mantiene en App.svelte porque el evento
 *  de la letra ya no contiene qué tecla Ctrl se mantuvo pulsada. */
export function matchesPaneShortcut(
    event: KeyboardEvent,
    shortcut: string,
    leftControlPressed: boolean,
): boolean {
    const normalized = normalizeShortcut(shortcut);
    if (!normalized) return false;
    return matchesShortcut(event, normalized)
        && (!normalized.split('+').includes('ctrl') || leftControlPressed);
}
