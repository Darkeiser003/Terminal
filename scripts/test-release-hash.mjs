...
const directory = await mkdtemp(join(tmpdir(), 'wterminal-release-hash-'));
...
try {
    ...
    await writeFile(manifest, [
        ...
        `${first}  WTerminal-Unpacked-1.0.0.zip`,
    ...
    result = await run(
        '--manifest', manifest,
        '--artifact', 'WTerminal-1.0.0-x64-setup.exe',
        '--hash', replacement,
    );
    assert.equal(result.code, 0, result.stderr);
    content = await readFile(manifest, 'utf8');
    assert.match(content, new RegExp(`${first}  WTerminal-Unpacked-1\\.0\\.0\\.zip`));
    assert.match(content, new RegExp(`${second}  LTerminal-1\\.0\\.0-x86_64\\.AppImage`));
    assert.match(content, new RegExp(`${replacement}  WTerminal-1\\.0\\.0-x64-setup\\.exe`));

    result = await run(
        '--manifest', manifest,
        '--artifact', 'WTerminal-Unpacked-1.0.0.zip',
        '--hash', replacement,
    );
    assert.equal(result.code, 0, result.stderr);
    content = await readFile(manifest, 'utf8');
    assert.match(content, /compilación local/, 'los comentarios Unicode deben conservarse');
    assert.equal((content.match(/WTerminal-Unpacked-1\.0\.0\.zip/g) ?? []).length, 1);
    assert.match(content, new RegExp(`${replacement}  WTerminal-Unpacked-1\\.0\\.0\\.zip`));
    assert.match(content, new RegExp(`${second}  LTerminal-1\\.0\\.0-x86_64\\.AppImage`));
