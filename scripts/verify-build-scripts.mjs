...
const checks = [
    ...
    ['Windows cruzado desde Linux publica la carpeta y el ZIP en el destino configurable release/ o releases/', files.linuxWindows.includes('package-windows-cross.mjs') && files.linuxWindows.includes('WinSlimTerminal-$VERSION_OVERRIDE$release_suffix') && files.linuxWindows.includes('WinSlimTerminal-Unpacked-$VERSION_OVERRIDE$release_suffix.zip') && files.linuxWindows.includes('LTERMINAL_RELEASE_DIR') && files.linuxWindows.includes('release_out="$release_out/dev"')],
    ...
    ['Los limpiadores protegen release/ y releases/ y no las borran como salidas generadas', files.cleanerPs1.includes('$ReleaseRoots') && files.cleanerPs1.includes('pertenece a release/ o releases/') && !cleanerPsDirectories.includes("'release'") && !cleanerPsDirectories.includes("'releases'") && files.cleanerSh.includes('release_root') && files.cleanerSh.includes('releases_root') && files.cleanerSh.includes('Ruta protegida (release/ o releases/)') && !cleanerShDirectories.includes(' release') && !cleanerShDirectories.includes(' releases')],
    ...
    ['La regresión del limpiador protege ambas carpetas de releases en Bash y PowerShell', files.package.includes('"test:cleaner": "node scripts/test-clean-repository.mjs"') && files.package.includes('npm run test:cleaner') && files.cleanerTest.includes("await symlink(source, join(project, 'dist'), 'dir')") && files.cleanerTest.includes("await symlink(externalData, join(config, 'winslim-terminal'), 'dir')") && files.cleanerTest.includes('releases/ conserva también sus Markdown empaquetados') && files.cleanerTest.includes('PowerShell conserva releases/') && files.cleanerTest.includes('el destino del enlace no se borra') && files.cleanerTest.includes('no se atraviesa el enlace de la carpeta de configuración')],
