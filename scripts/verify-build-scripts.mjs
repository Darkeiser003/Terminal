...
const checks = [
    ...
    ['Smoke Linux se ejecuta en una sesión propia y se cierra al terminar', files.linux.includes('setsid env') && files.linux.includes('cleanup_smoke_process') && files.linux.includes("trap 'restore_node_modules; cleanup_smoke_process' EXIT")],
    ...
    ['Linux aísla node_modules Windows y lo restaura al terminar', files.linux.includes('NODE_MODULES_RESTORE') && files.linux.includes('restore_node_modules') && files.linux.includes('@tauri-apps/cli-win32-x64-msvc') && files.linux.includes("trap 'restore_node_modules; cleanup_smoke_process' EXIT")],
